...
> Последнее обновление: 2026-07-13 (Stage 3 — **3/4 задачи завершены**, TD-S5-01/02/03 закрыты)
...
- **Stage 3 (Production-readiness):** 🔄 В РАБОТЕ (3/4) — TD-S5-01/02/03 ✅ закрыты
...
**TD-S5-04: Docker production** (MEDIUM приоритет)
- Multi-stage Dockerfile.app (builder + runtime), healthcheck для 1c-ai-app,
  .env.example, docker-compose.override.yml (dev hot reload).
- Архитектура: `docker/Dockerfile.app` (переписать на multi-stage),
  `.env.example` (все env vars), `docker-compose.override.yml` (dev),
  healthcheck для `1c-ai-app` (HTTP /health endpoint).
- **Фундамент готов:** TD-S5-01 (persistence + health_check()), TD-S5-02
  (Facade MCP server — /health можно добавить), TD-S5-03 (git MCP).
...
> **Задача:** Stage 3 (Production-readiness) — **TD-S5-01/02/03 ЗАВЕРШЕНЫ** ✅
> **Статус:** 3/4 закрыты (persistence + Facade handlers + git MCP)
> **Блокеры:** нет
...
- **Stage 3 прогресс:** 3/4 задач ЗАВЕРШЕНО ✅ (TD-S5-01/02/03 ✅; TD-S5-04 открыта)
- **Тесты:** 905 проходят + 7 skipped (3 BSL LS + 3 Postgres + 1 git integration)
...
- **Последний коммит:** TD-S5-03 git MCP — Stage 3 3/4
