...
> Последнее обновление: 2026-07-13 (Stage 3 — **2/4 задачи завершены**, TD-S5-01/02 закрыты)
...
- **Stage 3 (Production-readiness):** 🔄 В РАБОТЕ (2/4) — TD-S5-01/02 ✅ закрыты
...
**TD-S5-03: git MCP (4 tools)** (MEDIUM приоритет)
- subprocess git CLI. 4 tools: create_branch, commit, open_pr, diff.
- Архитектура: `packages/mcp_servers/src/mcp_servers/git/contracts.py` (уже есть),
  создать `packages/mcp_servers/src/mcp_servers/git/server.py` (аналогично bsl_ls/server.py).
- Auth: GitHub token из env (без хардкода). Безопасность: валидация branch names,
  проверка на secrets в diff.
- **Фундамент готов:** TD-S5-01 (persistence), TD-S5-02 (Facade handlers).
...
> **Задача:** Stage 3 (Production-readiness) — **TD-S5-01/02 ЗАВЕРШЕНЫ** ✅
> **Статус:** 2/4 закрыты (persistence + Facade handlers)
> **Блокеры:** нет
> **Что сделано:** ✅ PersistenceManager (TD-S5-01), ✅ FacadeHandlers с 8 tools + MCP
> server (TD-S5-02, DI через конструктор, in-memory state, ADR-0013 соблюдён)
> **Следующий шаг:** TD-S5-03 (git MCP, 4 tools) → TD-S5-04 (Docker production)
>
...
- **Stage 3 прогресс:** 2/4 задач ЗАВЕРШЕНО ✅ (TD-S5-01/02 ✅; TD-S5-03/04 открыты)
- **Тесты:** 846 проходят + 6 skipped (3 BSL LS + 3 Postgres integration, без Docker/PG)
...
- **Последний коммит:** TD-S5-02 Facade handlers — Stage 3 2/4
